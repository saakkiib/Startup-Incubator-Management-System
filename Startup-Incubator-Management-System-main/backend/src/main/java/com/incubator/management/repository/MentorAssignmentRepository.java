...
public interface MentorAssignmentRepository extends JpaRepository<MentorAssignment, Long> {
    ...
    @Modifying
    @Query("DELETE FROM MentorAssignment m WHERE m.mentor.id = :userId")
    void deleteByMentorId(Long userId);

    @Modifying
    @Query("DELETE FROM MentorAssignment m WHERE m.assignedBy.id = :userId")
    void deleteByAssignedById(Long userId);

    @Modifying
    @Query("DELETE FROM MentorAssignment m WHERE m.startup.id = :startupId")
